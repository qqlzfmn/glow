...
final class UsageBadgeTests {
    ...
    @Test func badgeShowsFirstOkProviderInOrder() {
        let usage = file([
            ...
            ("glm", "ok", UsageItem(label: "5 Hours", usedPercent: 42)),
        ])
        #expect(UsageBadge.badgeText(for: usage) == "5 Hours 42%")
    }
    ...
    @Test func badgeFallsBackToSortedKeysWithoutOrder() {
        var usage = file([
            ("zzz", "ok", UsageItem(label: "5 Hours", usedPercent: 7)),
            ("aaa", "ok", UsageItem(label: "5 Hours", usedPercent: 9)),
        ])
        usage.order = nil
        #expect(UsageBadge.badgeText(for: usage) == "5 Hours 9%")
    }

    // MARK: - itemText

    @Test func itemTextPercentAndBalance() {
        #expect(UsageBadge.itemText(UsageItem(label: "5 Hours", usedPercent: 42.4)) == "5 Hours 42%")
        ...
        #expect(UsageBadge.itemText(UsageItem(label: "Balance", remaining: 1200, unit: "M")) == "Balance M1200")
